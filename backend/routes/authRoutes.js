...
router.post("/login",async(req,res)=>{
    ...
    try{
        ...
        const token=jwt.sign({id:user.id,nombres:user.nombres,email:user.email,rol:user.rol},process.env.JWT_SECRET,{expiresIn:"12h"});
...
router.post("/forgot-password",async(req,res)=>{
    ...
    try {
        ...
        const resetToken=jwt.sign({id:user.id,nombres:user.nombres,email:user.email,rol:user.rol},process.env.JWT_SECRET,{expiresIn:"10m"});
        ...
        console.error(error);
        res.status(500).json({message:"Error en el envío del correo de recuperación."});
    }
})
